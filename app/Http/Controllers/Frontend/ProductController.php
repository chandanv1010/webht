...
class ProductController extends FrontendController
{
    ...
    public function demo(string $canonical = '', Request $request){
        $language = $this->language;
        ...
        $product_id = $this->router->module_id;
        $product = $this->productRepository->getProductById($product_id, $this->language, config('apps.general.defaultPublish'));
        if(is_null($product)){
            abort(404);
        }
        $config = $this->config();
        $template = 'frontend.product.product.demo';
        $system = $this->system;
        $seo = seo($product);
        return view($template, compact(
            'config',
