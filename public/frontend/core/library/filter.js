(function($) {
    ...
    HT.hideFilter = () => {
        const filtersHidden = sessionStorage.getItem('filtersHidden') === 'true';
        if (filtersHidden) {
            $('.panel-body').addClass('hide-filters');
        }
        $(document).on('click', '.filter-tools .toggle-filters', function() {
            const panelBody = $(this).closest('.panel-body');
            panelBody.toggleClass('hide-filters');
            const isHidden = panelBody.hasClass('hide-filters');
            $(this).find('.caption').text(isHidden ? 'Hiển thị bộ lọc' : 'Ẩn bộ lọc');
            sessionStorage.setItem('filtersHidden', isHidden);
        });
    };
