(function($) {
            ...
            setTimeout(() => {
                ...
                option.on = {
                    init: function () {
                        ...
                        document.querySelector('.panel-slide').classList.add('is-loaded');
                    }
                };
        ...
        $(document).on('click', '.widget-toc a', function(e) {
            ...
            if (href && href.startsWith('#')) {
                ...
                if (targetElement) {
                    ...
                    $('html, body').animate({
                        scrollTop: $targetElement.offset().top - 50 // Trừ 100px để tạo khoảng cách
                    }, 400); // 800ms cho hiệu ứng cuộn mượt
    ...
    HT.customer = () => {
        var swiper = new Swiper('.your-swiper-container', {
            loop: true,
            centeredSlides: true,
            slidesPerView: 1,         // Tổng cộng 3 + 0.5 + 0.5 = 4, nhưng sẽ giải thích bên dưới
            ...
            breakpoints: {
                768: {
                    slidesPerView: 1.5,     // 3 full + 0.75 + 0.75
                },
                1024: {
                    slidesPerView: 4,       // Nếu muốn 1/2 + 3 + 1/2 thì ta để là 5
                }
            }
        });
    };
    ...
    HT.manualSlide = () => {
        if ($('.panel-slide').length) {
            ...
            // Ensure wrapper is styled correctly
            $wrapper.css({
                'position': 'relative',
                'width': '100%',
                'height': '100%'
            ...
            const goToSlide = (newIndex) => {
                ...
                $slides.eq(currentSlide).removeClass('is-active');
                $slides.eq(newIndex).addClass('is-active');
            ...
            console.log('Manual slide system initialized.');
        }
    };
            ...
            if ($img.length) {
                function setupHoverEffect() {
                    ...
                    $container.off('mouseenter').on('mouseenter', function() {
                        ...
                        const imgHeight = $img[0].naturalHeight || $img[0].scrollHeight;
                        const imgWidth = $img[0].naturalWidth || $img[0].scrollWidth;
                        
                        // Tính toán chiều cao sau khi scale theo chiều rộng container
                        const scaledHeight = (imgHeight * containerWidth) / imgWidth;
                        
                        if (scaledHeight > containerHeight) {
                            const scrollDistance = scaledHeight - containerHeight;
                ...
                if ($img[0].complete && $img[0].naturalHeight !== 0) {
                    ...
                    $img.off('load').on('load', setupHoverEffect);
                }
            }
        });
    };
